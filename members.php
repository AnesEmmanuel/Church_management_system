    ...
    if ($community_id === 0 || $unit_id === 0 || $name === '' || $dob === '') {
        ...
        if (empty($err)) {
            $check_duplicate = $conn->prepare("SELECT id FROM members WHERE 
                (name = ? AND dob = ?) OR 
                (phone = ? AND phone != '') OR 
                (confirmation_no = ? AND confirmation_no != '') OR 
                (Baptism_no = ? AND Baptism_no != '') OR 
                (communial_no = ? AND communial_no != '') OR 
                (marriage_no = ? AND marriage_no != '')");
            
            $check_duplicate->bind_param("ssssssss", $name, $dob, $phone, $confirmation_no, $Baptism_no, $communial_no, $marriage_no);
            $check_duplicate->execute();
            $duplicate_result = $check_duplicate->get_result();
            
            if ($duplicate_result->num_rows > 0) {
                $duplicate_row = $duplicate_result->fetch_assoc();
                $err = "❌ Duplicate entry found! A member with the same information already exists (ID: " . $duplicate_row['id'] . ")";
            }
...
function checkDuplicate(field, value, fieldType) {
    ...
    fetch('check_duplicate.php', {
        ...
        body: `field=${field}&value=${encodeURIComponent(value)}&type=${fieldType}`
    })
    ...
    .then(data => {
        if (data.duplicate) {
            ...
            warning.innerHTML = `⚠️ ${data.message}`;
...
function validateForm() {
    ...
    for (const warning of warnings) {
        if (warning.style.display === 'block') {
            alert('Please fix duplicate entries before submitting.');
            isValid = false;
            break;
        }
    }
